...
if page == "Dashboard":
    ...
    with col1:
        city = st.selectbox(
            "City",
            ["All"] + sorted(
                students["City"].unique().tolist()
            )
        )

        filtered_students = filter_by_city(
            filtered_students,
            city
        )
    ...
    with col2:
        student_class = st.selectbox(
            "Class",
            ["All"] + sorted(
                students["Class"].unique().tolist()
            )
        )

        filtered_students = filter_by_class(
            filtered_students,
            student_class
        )
    ...
    filtered_students = filter_by_marks(
        filtered_students,
        marks
    )
    ...
    filtered_students = sort_students(
        filtered_students,
        sort_column,
        ascending
    )
    ...
    st.write(
        "Students performance analytics will appear here."
    )
